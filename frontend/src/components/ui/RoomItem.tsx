...
export default function RoomItem({ id, quantity, compleat }: BundleItem) {
  ...
  const classStyle = compleat
    ? 'bg-green-500 relative border border-green-600 rounded-md p-2 mt-1'
    : 'relative border rounded-md p-2 mt-1'

  return (
    <div className={classStyle}>
      <Image src={imageSrc} width={40} height={40} alt={'Bundle Image'} />
      {quantity < 100 && quantity > 1 && (
        <span className="absolute flex items-center justify-center -right-1.5 -top-1.5 border rounded-full w-6 h-6 text-sm bg-white">
          {quantity}
        </span>
      )}
    </div>
